    ...
    export class CellFactory {
        ...
        public updateCell(panel: GridPanel, r: number, c: number, cell: HTMLElement, rng?: CellRange, updateContent?: boolean) {
            ...
            if (ct == CellType.Cell) {
                ...
                if (g.editRange && col.dataType != DataType.Boolean && g.editRange.contains(r, c)) {
            ...
            if (updateContent != false) {
                ...
                if (ct == CellType.Cell && c == g.columns.firstVisibleIndex &&
                    ...
                    if (isNumber(val) && !col.dataMap) {
                        content = Globalize.format(val, 'g12');
                    }
        ...
        select(rng: any, show: any = true) {
            ...
            switch (g.selectionMode) {
                ...
                case SelectionMode.ListBox:
                    ...
                    g.refreshCells(false, true, [new CellRange(0, 0, g.rows.length - 1, g.columns.length - 1)]);
                    break;
            }
    ...
    export class _MouseHandler {
        ...
        private _mouseDown(e: MouseEvent) {
            ...
            if (e.target != document.activeElement && !g.containsFocus()) {
                if (ht.cellType == CellType.Cell && e.target instanceof HTMLElement) {
            ...
            grid.addEventListener(grid.hostElement, 'mousedown', function (e) {
                ...
                if (ht.cellType != CellType.Cell && ht.cellType != CellType.None) {
                    ...
                    var edt = <HTMLInputElement>tryCast(e.target, HTMLInputElement),
                        parent = edt ? edt.parentElement : null;
                    if (edt && edt.type == 'checkbox' && // checkbox
                        parent && hasClass(parent, 'wj-cell') && // in a cell
                        parent.parentElement == grid.cells.hostElement) { // on this grid
