...
module wijmo.input {
    ...
    export class ListBox extends Control {
         ...
         */
        static controlTemplate = '<div wj-part="lb-content"></div>';
        ...
        set checkedItems(value: any[]) {
            ...
            if (this._cv) {
                for (var i = 0; i < this._cv.items.length; i++) {
                    this.setItemChecked(i, arr.indexOf(i) > -1);
                }
        ...
        private _click(e: MouseEvent) {
            ...
            for (var index = 0; index < this._div.childElementCount; index++) {
                if (contains(this._div.childNodes[index], e.target)) {
                    this.selectedIndex = index;
                    break;
                }
            }
        ...
        private _keypress(e: KeyboardEvent) {
            if (!e.defaultPrevented) {
                ...
                // spacebar toggles current item
                if (e.charCode == 32 && this.checkedMemberPath && this.selectedIndex > -1) {
                    var cb = this._getCheckbox(this.selectedIndex);
                    if (cb) {
                        e.preventDefault();
                        this.setItemChecked(this.selectedIndex, !cb.checked);
...
module wijmo.input {
    ...
    export class ComboBox extends DropDown {
        ...
        constructor(element: any, options?) {
            ...
            // handle cursor keys
            this.addEventListener(this._tbx, 'keydown', this._handleKeyDown.bind(this));
    ...
    export class AutoComplete extends ComboBox {
        ...
        _setText(text: string) {
            ...
            if (text != this._oldText) {
                ...
                // assign only if necessary to prevent sionally swapping chars (Android 4.4.2)
...
module wijmo.input {
    ...
    export class Menu extends ComboBox {
        ...
        constructor(element: any, options?) {
            ...
            var self = this;
            this.addEventListener(this._header, 'click', function () {
                if (self._isButton) {
                    self.isDroppedDown = false;
                    if (!self.selectedItem) {
                        self.selectedItem = self._defaultItem;
                    }
            ...
            // handle Escape key (since _tbx input is hidden)
            self.addEventListener(self.hostElement, 'keydown', function (e) {
                if (e.keyCode == Key.Escape) {
                    self.isDroppedDown = false;
                    if (!self.selectedItem) { // TFS 119513
                        self.selectedItem = self._defaultItem;
                    }
                }
            });

            // handle Escape key when the list has focus (ContextMenu, TFS 120714)
            self.addEventListener(self._dropDown, 'keydown', function (e) {
                if (e.keyCode == Key.Escape) {
                    self.isDroppedDown = false;
                }
            });
...
module wijmo.input {
    ...
    export class InputColor extends DropDown {
        ...
        constructor(element: any, options?) {
            ...
            // handle keyboard
            var self = this;
            this.addEventListener(this._tbx, 'keydown', function (e) {
                switch (e.keyCode) {
                    case Key.Enter:
                        self._commitText();
                        break;
