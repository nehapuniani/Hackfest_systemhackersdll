...
module wijmo.grid.filter {
    ...
    export class FlexGridFilter {
        ...
        getColumnFilter(col: Column, create = true): ColumnFilter {
            ...
            if (isString(col)) {
                col = this._grid.columns.getColumn(col.toString());
            }
                ...
                if (cf) {
                    switch (cfs.type) {
                        case 'condition':
                            ...
                            cfc.condition1.value = changeType(cfs.condition1.value, col.dataType, col.format);
                            cfc.and = cfs.and;
                            cfc.condition2.operator = cfs.condition2.operator;
                            cfc.condition2.value = changeType(cfs.condition2.value, col.dataType, col.format);
                            break;
                        case 'value':
                            var cfv = cf.valueFilter;
        ...
        editColumnFilter(col: any) {
            ...
            // close editor when editor button is clicked or when it loses focus
            ...
            if (this._hasAttribute(e.target, FlexGridFilter._WJA_FILTER)) {
                ...
                if (ht.panel == this._grid.columnHeaders) {
                    ...
                    if (this._divEdt && this._edtCol == col) {
                        ...
                        setTimeout(function () { // allow some time to close editors (TFS 117746)
